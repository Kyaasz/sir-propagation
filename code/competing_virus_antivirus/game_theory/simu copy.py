##################################################################################################
### Fichier pour la génération de données pour le jeu virus-antivirus pour une seule stratégie ###
##################################################################################################
...
for k in K_tab: 
    ...
    for i in range(sim_nb):
        ep_temp, inf_tab = game.sneaky_competing_propagation(G, [0], 0, 1, 1, k)
        ep = ep_temp + ep
        tab.append(inf_tab)
    ep = ep/sim_nb
    EP_tab.append(ep)
...
with open('competing_virus_antivirus/game_theory/res_simu/cost_little_graph_unicast_sneaky_virus.csv', 'w', newline='') as f:
    writer = csv.writer(f)
