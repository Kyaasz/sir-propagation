######################################################################################
### Fichier de génération parallélisée de données pour le ML avec un unique graphe ###
######################################################################################
...
nb_simu = 8000 ### nombre de simulation par type de propagation

## génération du graphe et des caractéristiques
G = nx.read_gexf("graph_simu2.gexf", node_type=int)
...
### run parallel
debut = time.time()
resultats = Parallel(n_jobs=cpu_count())(delayed(simu)(p,i) for p in range(2,3,1) for i in range(nb_simu))
temps_ex = time.time() - debut
print("=========================================== Fin des simulations ===================================================")

## écriture du csv 
with open('resultats13.csv', 'w', newline='') as f:
    writer = csv.writer(f)
