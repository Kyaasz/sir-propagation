###############################################################################
### Fichier qui génère la simulation - appelé par app1 (ancienne stagiaire) ###
###############################################################################
...
def propagation_unicast(begin, G, q, resist, alpha, prob, nbr, nbr_smart):
   ...
   for n in range(1):  
        ...
        # initialement, aucun sommet n'a été traité
        for n in G.nodes:
            G.nodes[n]['visited'] = False
        #print('nodes',G.nodes)
        
        ### on remplit la liste des voisins avec tous les voisins de tous les sommets
        for n in G.nodes:
            liste_neighbors.append(list(G.neighbors(n)))
        
        ### edge = tableau des index des arêtes à traiter
        for i in G.nodes:
                edge = []
        ...
        while ((0  in state) or  (1  in state)):  ### condition à discuter
                    ### setup   
                    liste = [0] * len(state)
                    liste_SR = [0] * len(state)
                    nv_node_liste = []
                    node_liste = [] ### noeuds infectés
...
def honey_pot(G,nodes_out,nodes_in,resist,list_edge,liste_resist,nbr_honey,nbr_honey_smart):
    ...
    x = nbr_honey-nbr_honey_smart ### forcément positif ou nul
    if tail_smart == 0 :
        
        if taille >=nbr_honey:
            
            hosp=random.sample(edge_total, k=nbr_honey) ### mettre des min au lieu de if
        ...
        if taille >=nbr_honey:
            ...
            if tail_smart >= z:
                ...
                honey_smart= random.choices(edge_smart,weights=len_smart,k=z) ### Dans ce cas de figure, ou le nombre d'aretes est inférieur au nb de honeypots voulu, peutetre privilégier les smarts aux normaux ? 
