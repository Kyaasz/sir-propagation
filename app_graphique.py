...
def citer_malades(event=None):
    ...
    if (choix_int > G.number_of_nodes()):
        ...
        if nb_inf > 0:
            ...
            framespinm.pack()
            l = Label(framespinm, text="Entrer les indices des noeuds infectés")
            l.pack()
...
def citer_resistants(event=None):
    ...
    if (choix_int > G.number_of_nodes() - nb_inf):
        ...
        if nb_res >0:
            framespinr.pack()
            l = Label(framespinr, text="Entrer les indices des noeuds résistants")
            l.pack()
            for i in range(int(choix)):
...
label2 = Label(frame2, text="Construire un nouveau graphe ou utiliser l'ancien ?")
label2.pack()
value2 = StringVar()
